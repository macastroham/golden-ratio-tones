...
// Precision Mathematical Entrainment Parameters Array Manifest Map
const NEURO_PRESETS = {
...
// Initialize the Unified Audio Ecosystem Graph
function initAudio() {
    if (audioCtx) return; 
    ...
    // Secure Pipeline Mapping Connections Hook Routing
    distortionNode.connect(spaceFilterNode);
    spaceFilterNode.connect(masterGainNode);
    masterGainNode.connect(analyserNode);
    analyserNode.connect(audioCtx.destination);
    
    // Instantiate both frame rendering animation loop paths concurrently
    drawOscilloscope();
    drawSafeMandala();
}
...
// HIGH-SATURATION DATA-BACKED PHOTIC COLOUR FIELD ENGINE
function drawSafeMandala() {
    ...
    function render() {
        ...
        // Semi-transparent overlay clear loop handles the fluid movement trails
        ctx.fillStyle = 'rgba(13, 15, 18, 0.08)';
        ctx.fillRect(0, 0, canvas.width, canvas.height);
        
        const voiceCount = Object.keys(activeVoices).length;
        const centerX = canvas.width / 2, centerY = canvas.height / 2, maxRadius = Math.min(centerX, centerY) * 1.1;
        
        if (!photicVisualsEnabled || voiceCount === 0) {
            ctx.fillStyle = 'rgba(251, 191, 36, 0.02)';
            ctx.beginPath(); ctx.arc(centerX, centerY, maxRadius * 0.15, 0, Math.PI * 2); ctx.fill();
            return;
        }
        ...
        // Set pixel composite rules to 'screen' to enable mathematical light layer stacking
        ctx.globalCompositeOperation = 'screen';

        // Stacks 3 distinct radial gradient globes to saturate retinal peripheral fields safely
        for (let layer = 1; layer <= 3; layer++) {
            const phaseShift = layer * (Math.PI / 3);
            const expansion = 0.4 + Math.sin(pulsePeriod + phaseShift) * 0.25;
            const currentRadius = maxRadius * expansion;
            
            if (currentRadius <= 0) continue;

            const hue = Math.round((baseHue + (layer * 120)) % 360);
            const gradient = ctx.createRadialGradient(centerX, centerY, 0, centerX, centerY, currentRadius);
            
            gradient.addColorStop(0, `hsla(${hue}, 90%, 55%, 0.22)`);
            gradient.addColorStop(0.5, `hsla(${(hue + 40) % 360}, 85%, 45%, 0.10)`);
        ...
        // Return compositing mode securely back to base parameter settings
        ctx.globalCompositeOperation = 'source-over';

        // Floating focal point center tracking ring
        ctx.strokeStyle = `hsla(${baseHue}, 70%, 65%, 0.15)`; ctx.lineWidth = 1 * window.devicePixelRatio;
        ctx.beginPath();
        ctx.arc(centerX, centerY, maxRadius * 0.3 * (0.8 + Math.sin(pulsePeriod) * 0.1), 0, Math.PI * 2);
        ctx.stroke();
    }
    render();
}
...
// System Hardware Wire Event Hooks Bindings
